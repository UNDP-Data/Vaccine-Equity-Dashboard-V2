...
import {
  CONTINENTS, HDI_LEVELS, INCOME_GROUPS, LABEL_EXTRA,
} from '../Constants';
...
export const HorizontalBarChart = (props: Props) => {
  ...
  const dataFormatted = orderBy(
    ...
    }).filter((d) => d.xVal !== undefined && d.xVal !== null && d.country && d.countryGroup && d.incomeGroup && d.region), 'xVal', reverseOrder ? 'desc' : 'asc',
  );
    ...
    : colorIndicator === 'Income Groups' ? INCOME_GROUPS
      : colorIndicator === 'Human development index (HDI)' ? [0.55, 0.7, 0.8]
        : colorIndicatorMetaData?.Categories ? colorIndicatorMetaData?.Categories
          : [0, 0];
  const colorScale = colorIndicator === 'Human development index (HDI)' ? scaleThreshold<string | number, string>().domain(colorDomain).range(UNDPColorModule.divergentColors.colorsx04).unknown(UNDPColorModule.graphGray) : scaleOrdinal<string | number, string>().domain(colorDomain).range(colorList).unknown(UNDPColorModule.graphGray);
    ...
    <div className='undp-scrollbar'>
      <svg width='100%' viewBox={`0 0 ${svgWidth} ${svgHeight}`}>
        ...
        >
          ...
          {
            colorIndicator === 'Human development index (HDI)' ? UNDPColorModule.divergentColors.colorsx04.map((d, i) => (
              <g
